...
public class Utilities {
    ...
    public static void calculateStats(long[] latencies, String requestType) {
        Arrays.sort(latencies);
        long sum = 0;
        for (long latency : latencies) {
            sum += latency;
        }
        double mean = (double) sum / latencies.length;
        double median = latencies[latencies.length / 2];
        double p99 = latencies[(int) Math.ceil(0.99 * latencies.length) - 1];
